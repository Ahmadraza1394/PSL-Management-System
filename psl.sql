...
INSERT INTO USER_INFO (pname, username, passwords) VALUES('Jane Smith', 'jane_smith', 'secure456');

-- Create a view to display user information
CREATE VIEW user_view AS
SELECT pname, username FROM USER_INFO;
...
FOR EACH ROW
BEGIN
    INSERT INTO user_log (log_message) VALUES (CONCAT('New user added: ', NEW.pname, ' (', NEW.username, ')'));
END;
...
Create table Team(
 TEAM VARCHAR(255),
    ...
    MEDIA_PARTNER VARCHAR(255),
    CAPTAIN VARCHAR(255),
    COACH VARCHAR(255),
    TeamLevel VARCHAR(255));
...
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Multan Sultan','Ali Tareen and Alamgir Tareen',41000000,'Multan,Punjab','Dawn News','Shan Masood','Andy Flower','Starter');
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Lahore Qalandar','Fawad Rana',24000000,'Lahore,Punjab','Geo Groups','Bredon McCullum','Aaqib Javed','Intermediate');
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Quetta Gladiators','Nadeen Omer',11000000,'Quetta,Balochistan','Ptv Sports','Sarfaraz Ahmad','Moin Khan','Intermediate');
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Peshawar Zalmi','Jawaid Afridi',16000000,'Peshawar,KPK','Express News','Darren Sammy','Muhammad Akram','Starter');
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Islamabad United','Ali and Amna Naqvi',15000000,'Capital Teritory','Dunya News','Misbah-ul-Haq','Dane Jones','Pro');
Insert into SYSTEM.TEAM (TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel) values ('Karachi Kings','Salman Iqbal',26000000,'Karachi,Sindh','Ary News','Imad Wasim','Micky Arthur','Pro');



--Making view for team because we are accessing data from view
create or replace view Team_v (TEAMName,OWNERName,Amount,TeamPROVIENCE,PlayerMEDIA_PARTNER,TeamCAPTAIN,TeamCOACH,OverallTeamLevel) AS select TEAM,OWNER,PRICE,PROVINCE,MEDIA_PARTNER,CAPTAIN,COACH,TeamLevel from TEAM;
select * from Team_v;
...
UPDATE TeamManagers
SET TeamSelector = 'Shahid Afridi', coachNationality = 'England'
WHERE CoachID = 1 AND CoachName = 'Andy Flower';

UPDATE TeamManagers
SET TeamSelector = 'Shoaib Akhtar', coachNationality = 'Pakistani'
WHERE CoachID = 2 AND CoachName = 'Aaqib Javed';

UPDATE TeamManagers
SET TeamSelector = 'Kamran Akmal', coachNationality = 'Pakistani'
WHERE CoachID = 3 AND CoachName = 'Moin Khan';

UPDATE TeamManagers
SET TeamSelector = 'Umar Akmal', coachNationality = 'Pakistani'
WHERE CoachID = 4 AND CoachName = 'Muhammad Akram';

UPDATE TeamManagers
SET coachname='David Warner', coachNationality = 'England'
WHERE CoachID = 5 AND CoachName = 'Dane Jones';
commit;
UPDATE TeamManagers
SET TeamSelector = 'Imam-ul-Haq', coachNationality = 'Australia'
WHERE CoachID = 6 AND CoachName = 'Dane Jones';
...
CREATE TABLE PLAYERS (
    ...
    city VARCHAR(255),
    phistory VARCHAR(50),
    playerfitnessrecord VARCHAR(255)
);
...
select P_name,p_type from players WHERE TEAM='Peshawar Zalmi';
...
-----------------------------Procedure
CREATE OR REPLACE PROCEDURE get_player_info(p_id_param IN NUMBER)
AS
    v_player_name players.p_name%TYPE;
    v_player_type players.p_type%TYPE;
BEGIN
    SELECT p_name, p_type INTO v_player_name, v_player_type
    FROM players
    WHERE P_ID = p_id_param;

    DBMS_OUTPUT.PUT_LINE('Player Name: ' || v_player_name);
    DBMS_OUTPUT.PUT_LINE('Player Type: ' || v_player_type);
END get_player_info;
...
Insert into SYSTEM.MATCHes (M_NO,TEAM,M_DATE,VENUE,matchtype) values (34,' Eliminator 1: Eliminator Winner Vs Qualifier Runner Up',to_date('21-MAR-20','DD-MON-RR'),'Lahore-Night','T-20');

Insert into SYSTEM.MATCH (M_NO,TEAM,M_DATE,VENUE,matchtype) values (35,'Final',to_date('22-MAR-20','DD-MON-RR'),'Lahore','T-20');
...
SELECT MATCHes.M_NO,MATCHes.team, MATCHes.matchtype,Sceheduleeee.time,Sceheduleeee.datee,Sceheduleeee.place FROM MATCHes INNER JOIN Sceheduleeee ON MATCHes.M_NO=Sceheduleeee.M_NO ORDER BY MATCHes.M_NO;
...
CREATE TABLE GROUNDS (
    G_NAME VARCHAR2(255),
    G_LOCATION VARCHAR2(255),
    G_CAPACITY NUMBER,
    MediaPartner VARCHAR2(255),
    Pitchno NUMBER,
    PRIMARY KEY (G_NAME)
);


Insert into GROUNDs (G_NAME,G_LOCATION,G_CAPACITY,MediaPartner,Pitchno) values ('Gaddafi Stadium','Lahore',27000,'Geo News',1);
Insert into GROUNDs (G_NAME,G_LOCATION,G_CAPACITY,MediaPartner,Pitchno) values ('National Stadium','Karachi',34228,'Ary News',2);
Insert into GROUNDs (G_NAME,G_LOCATION,G_CAPACITY,MediaPartner,Pitchno) values ('Rawalpindi Stadium','Rawalpindi',10000,'Geo News',1);
Insert into GROUNDs (G_NAME,G_LOCATION,G_CAPACITY,MediaPartner,Pitchno) values ('Multan Stadium','Multan',13000,'Bol News',2);
select * from grounds;
